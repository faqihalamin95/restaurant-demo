...
-- 2. Order Performance (vs 30h Sebelumnya)
ord AS (
    SELECT ROUND((curr - prev) / NULLIF(prev, 0) * 100, 1) AS p
    FROM (
        SELECT
            SUM(CASE WHEN order_date >= (SELECT d FROM anchor_date) - INTERVAL '29 days' THEN total_orders ELSE 0 END) AS curr,
            SUM(CASE WHEN order_date >= (SELECT d FROM anchor_date) - INTERVAL '59 days' AND order_date < (SELECT d FROM anchor_date) - INTERVAL '29 days' THEN total_orders ELSE 0 END) AS prev
...
-- 7. Member Order
mem_stat AS (
    SELECT ROUND(SUM(total_orders)*100.0/NULLIF((SELECT SUM(total_orders) FROM main_marts.mart_daily_revenue WHERE order_date >= (SELECT d FROM anchor_date) - INTERVAL '29 days' AND order_date <= (SELECT d FROM anchor_date)),0),1) AS p
...
emp_stat AS (
    ...
    WHERE attendance_date >= (SELECT d FROM anchor_date) - INTERVAL '29 days'
      AND attendance_date <= (SELECT d FROM anchor_date)
),
-- 11 & 12. Inventori
inv_stat AS (
    SELECT
        ROUND(SUM(usage_cost)*100.0/NULLIF((SELECT SUM(gross_revenue) FROM main_marts.mart_daily_net_revenue WHERE metric_date >= (SELECT d FROM anchor_date) - INTERVAL '29 days' AND metric_date <= (SELECT d FROM anchor_date)),0),1) AS b,
        ROUND(SUM(purchase_cost)/NULLIF(SUM(usage_cost),0),2) AS r
    FROM main_marts.mart_inventory_stok
    WHERE txn_date >= (SELECT d FROM anchor_date) - INTERVAL '29 days'
...
UNION ALL
SELECT 'Cabang', '🏪',
    CASE WHEN p < -15 THEN 'kritis' WHEN p < -5 THEN 'perhatian' ELSE 'sehat' END,
    'Order ' || p || '% vs 30h lalu', 'Order vs 30h Lalu' FROM ord
UNION ALL
SELECT 'Cabang', '🏪',
