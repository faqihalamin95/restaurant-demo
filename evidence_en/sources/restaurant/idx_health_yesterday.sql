...
-- 1. Keuangan
fin AS (
    SELECT ROUND((curr - avg_sdow) / NULLIF(avg_sdow, 0) * 100, 1) AS p
    FROM (
        SELECT
            SUM(CASE WHEN order_date = (SELECT d FROM anchor_date) THEN total_revenue ELSE 0 END) AS curr,
            AVG(CASE WHEN order_date < (SELECT d FROM anchor_date)
                ...
                THEN total_revenue END) AS avg_sdow
...
ord AS (
    ...
    FROM (
        SELECT
            SUM(CASE WHEN order_date = (SELECT d FROM anchor_date) THEN total_orders ELSE 0 END) AS curr,
            AVG(CASE WHEN order_date < (SELECT d FROM anchor_date)
                AND DAYOFWEEK(order_date) = DAYOFWEEK((SELECT d FROM anchor_date))
                AND order_date >= (SELECT d FROM anchor_date) - INTERVAL '30 days'
                THEN total_orders END) AS avg_sdow
        FROM main_marts.mart_daily_revenue
        WHERE order_date >= (SELECT d FROM anchor_date) - INTERVAL '30 days'
    )
),
...
-- 4. Gap
gap_stat AS (
    SELECT ROUND((MAX(total_revenue)-MIN(total_revenue))/NULLIF(MIN(total_revenue),0)*100,1) AS g
    FROM main_marts.mart_daily_revenue
    WHERE order_date = (SELECT d FROM anchor_date)
),
-- 5. Menu
...
-- 6. Member
mem_stat AS (
    SELECT ROUND(SUM(total_orders)*100.0/NULLIF((SELECT SUM(total_orders) FROM main_marts.mart_daily_revenue WHERE order_date = (SELECT d FROM anchor_date)),0),1) AS p
...
inv_stat AS (
    SELECT
        ROUND(SUM(usage_cost)*100.0/NULLIF((SELECT SUM(gross_revenue) FROM main_marts.mart_daily_net_revenue WHERE metric_date = (SELECT d FROM anchor_date)),0),1) AS b,
        ROUND(SUM(purchase_cost)/NULLIF(SUM(usage_cost),0),2) AS r
    FROM main_marts.mart_inventory_stok
    WHERE txn_date = (SELECT d FROM anchor_date)
),
-- 11. Jam Sibuk
peak_stat AS (
...
SELECT 'Keuangan' AS section, '💰' AS icon,
    ...
    'Revenue kemarin ' || p || '% vs SDOW' AS label, 'vs Hari Serupa' AS metrik FROM fin
UNION ALL
SELECT 'Cabang', '🏪',
    CASE WHEN p >= -5 THEN 'sehat' WHEN p >= -15 THEN 'perhatian' ELSE 'kritis' END,
    'Jumlah order ' || p || '% vs SDOW', 'Order vs SDOW' FROM ord
UNION ALL
SELECT 'Cabang', '🏪',
...
SELECT 'Menu', '🍽️',
    ...
    'Menu terlaris dominasi ' || k || '%', 'Kontribusi Menu' FROM menu_stat
UNION ALL
SELECT 'Member', '👥',
    CASE WHEN p < 20 THEN 'kritis' WHEN p < 40 THEN 'perhatian' ELSE 'sehat' END,
    p || '% order dari member', 'Order Member' FROM mem_stat
UNION ALL
SELECT 'Pegawai', '👨‍💼',
...
UNION ALL
SELECT 'Inventori', '📦',
    CASE WHEN r > 1.5 THEN 'kritis' WHEN r = 0 THEN 'sehat' WHEN r < 0.9 THEN 'kritis' WHEN r > 1.2 THEN 'perhatian' ELSE 'sehat' END,
    'Rasio beli/pakai ' || r, 'Rasio Beli/Pakai' FROM inv_stat
UNION ALL
SELECT 'Jam Sibuk', '⏰',
    CASE WHEN p > 20 THEN 'kritis' WHEN p > 12 THEN 'perhatian' ELSE 'sehat' END,
    p || '% order di jam puncak', 'Konsentrasi Order' FROM peak_stat
