---
title: "Wekadata: Business Performance Summary"
sidebar_link: true
---

<script>
  const depts = [
    { id: 'Location', name: 'Branch Performance', icon: '🏪', link: '/02-branch-performance', desc: 'Daily outlet revenue, profitability, and margin comparison audit.' },
    { id: 'Inventory', name: 'Stock Inventory', icon: '📦', link: '/03-inventory', desc: 'Monitor raw material availability, reorder levels, and stock movement.' },
    { id: 'Jam Sibuk', name: 'Peak Hours', icon: '⏰', link: '/04-peak-hours', desc: 'Operational hours mapping, staff optimization, and restaurant capacity analysis.' },
    { id: 'Menu', name: 'Menu Performance', icon: '🍔', link: '/05-menu-performance', desc: 'Top selling menu analysis, dead stock detection, and BCG matrix evaluation.' },
    { id: 'Member', name: 'Member Insights', icon: '💎', link: '/06-member-behavior', desc: 'Customer transaction pattern, retention, and loyalty tier performance analysis.' },
    { id: 'Pegawai', name: 'Employee Performance', icon: '👥', link: '/07-employee-performance', desc: 'Productivity, attendance, overtime, and restaurant team efficiency evaluation.' }
  ];
  ...
  const getSyncedDesc = (d, branch_status, inv_overview, peak_vol, menu_health, member_kpi, workforce_overview, inv_branch_status) => {
    if (d.id === 'Location' && branch_status && branch_status.length > 0) {
      let count = branch_status[0].sehat_count + branch_status[0].recovery_count;
      if (count === 4) return 'All branches are operating optimally with healthy margins.';
      if (count === 3) return 'Majority of branches are exhibiting healthy and safe financial conditions.';
      if (count === 2) return 'Half of the branches are experiencing margin pressure. Active monitoring required.';
      return 'Majority of branches are currently under severe operational margin pressure.';
    }
    if (d.id === 'Inventory' && inv_branch_status && inv_branch_status.length > 0) {
      let healthyBranches = inv_branch_status[0].sehat_count;
      if (healthyBranches === 4) return 'All branch inventories are fully optimized. No stock crisis detected.';
      if (healthyBranches >= 2) return 'Inventory levels are generally controlled. Monitor branches nearing critical stock limits.';
      return 'Critical supply chain issues detected. Immediate stock replenishment required.';
    }
    if (d.id === 'Jam Sibuk' && peak_vol && peak_vol.length > 0) {
      let lvl = peak_vol[0].volatility_level;
      if (lvl === 'tinggi') return 'High Volatility: Traffic patterns are unpredictable. Daily deviation: ±' + peak_vol[0].cv_pct + '%.';
      if (lvl === 'sedang') return 'Moderate Volatility: Demand occasionally fluctuates. Daily deviation: ±' + peak_vol[0].cv_pct + '%.';
      return 'Stable Demand: Traffic patterns are highly predictable. Daily deviation: ±' + peak_vol[0].cv_pct + '%.';
    }
    if (d.id === 'Menu' && menu_health && menu_health.length > 0) {
      if (menu_health[0].declining_30d >= 5) return `Critical Alert: ${menu_health[0].declining_30d} out of ${menu_health[0].active_30d} active items show >20% revenue drop.`;
      if (menu_health[0].declining_30d >= 2) return `Warning: ${menu_health[0].declining_30d} out of ${menu_health[0].active_30d} active items exhibit significant revenue decline.`;
      return `Stable Performance: The majority of ${menu_health[0].active_30d} active items show consistent sales volume.`;
    }
    if (d.id === 'Member' && member_kpi && member_kpi.length > 0) {
      return `Customer base is active. Total enrolled members: ${member_kpi[0].total_members}. Retention patterns align with standard benchmarks.`;
    }
    if (d.id === 'Pegawai' && workforce_overview && workforce_overview.length > 0) {
      let rate = workforce_overview[0].attendance_30d;
      if (rate >= 92) return 'Workforce efficiency is optimal. The majority of scheduled shifts are successfully fulfilled.';
      if (rate >= 85) return 'Attendance is satisfactory, but emerging fulfillment gaps require attention.';
      return 'Critical workforce shortage. High rate of unfulfilled shifts poses operational risks.';
    }
    return d.desc;
  };

  const getSyncedStatus = (d, health_30d, branch_status, inv_overview, peak_vol, menu_health, member_kpi, retention_kpi, workforce_overview, inv_branch_status) => {
    if (d.id === 'Location' && branch_status && branch_status.length > 0) {
      let count = branch_status[0].sehat_count + branch_status[0].recovery_count;
    ...
    if (d.id === 'Inventory' && inv_branch_status && inv_branch_status.length > 0) {
      let healthyBranches = inv_branch_status[0].sehat_count;
      return healthyBranches === 4 ? 'sehat' : healthyBranches >= 2 ? 'waspada' : 'kritis';
    }
    if (d.id === 'Jam Sibuk' && peak_vol && peak_vol.length > 0) {
      let lvl = peak_vol[0].volatility_level;
...
SELECT 
    CASE WHEN net_margin_pct < 15 THEN 'Critical' ELSE 'Healthy' END as margin_status
FROM fin
```
...
```sql milestone_count
SELECT COUNT(*) as count FROM restaurant.milestone_queue
```
...
<p style="margin-top: 12px; margin-bottom: 32px; color: var(--color-text-secondary);"><em>Business intelligence dashboard to help Business Owners monitor operational health, profitability, and business trends.</em></p>
...
{#if hud_finance && hud_finance.length > 0 && health_30d && health_30d.length > 0}
  ...
  <div class="hero-health-card status-{status}" style="cursor: pointer;" onclick="window.location.href='/01-financial-report'">
...
<h2 class="hero-card-title">
      {#if status === 'sehat'}
        🎉 Healthy Financial (Target 10% Achieved)
      {:else if status === 'waspada'}
        ⚠️ Financial Slowing Down (Below Target 10%)
      {:else}
        🚨 Critical Financial (Margin Severely Eroded)
      {/if}
    </h2>
    ...
    <p class="hero-card-desc" style="color: #000000; margin-bottom: 12px;">
      ...
      {:else if status === 'waspada'}
        Margin has entered the warning zone in 30 days.
      {:else}
        Margin is operationally critical.
      {/if}
    </p>
    
    <div class="hero-click-hint">
      View Financial Report Details <span>→</span>
    </div>
  </div>
{/if}
...
<div class="portal-grid">
  {#if health_30d}
    {#each depts as d}
      ...
      {@const s_desc = getSyncedDesc(d, typeof branch_status !== 'undefined' ? branch_status : null, typeof inv_overview !== 'undefined' ? inv_overview : null, typeof peak_vol !== 'undefined' ? peak_vol : null, typeof menu_health !== 'undefined' ? menu_health : null, typeof member_kpi !== 'undefined' ? member_kpi : null, typeof workforce_overview !== 'undefined' ? workforce_overview : null, typeof inv_branch_status !== 'undefined' ? inv_branch_status : null)}
      {@const s_status = getSyncedStatus(d, health_30d, typeof branch_status !== 'undefined' ? branch_status : null, typeof inv_overview !== 'undefined' ? inv_overview : null, typeof peak_vol !== 'undefined' ? peak_vol : null, typeof menu_health !== 'undefined' ? menu_health : null, typeof member_kpi !== 'undefined' ? member_kpi : null, typeof retention_kpi !== 'undefined' ? retention_kpi : null, typeof workforce_overview !== 'undefined' ? workforce_overview : null, typeof inv_branch_status !== 'undefined' ? inv_branch_status : null)}

      {@const c_sehat = typeof branch_status !== 'undefined' && branch_status.length > 0 ? branch_status[0].sehat_count + branch_status[0].recovery_count : 0}
      ...
      {@const m_special = typeof milestone_count !== 'undefined' && milestone_count.length > 0 && milestone_count[0].count > 0 ? 1 : 0}
      ...
      <a href="{d.link}" class="portal-card status-{s_status}" data-sveltekit-reload>
        ...
        {#if d.id === 'Location'}
          <div style="position: absolute; top: 16px; right: 16px; display: flex; gap: 10px; font-size: 0.85rem; font-weight: 700; background: rgba(0,0,0,0.04); border: 1px solid rgba(0,0,0,0.06); padding: 4px 12px; border-radius: 99px;">
            <span style="color: #15803d;" title="{c_sehat} Healthy Branches">✓ {c_sehat}</span>
            <span style="color: #b45309;" title="{c_waspada} Warning Branches">! {c_waspada}</span>
            <span style="color: #b91c1c;" title="{c_kritis} Critical Branches">x {c_kritis}</span>
          </div>
        {/if}
        {#if d.id === 'Inventory'}
          <div style="position: absolute; top: 16px; right: 16px; display: flex; gap: 10px; font-size: 0.85rem; font-weight: 700; background: rgba(0,0,0,0.04); border: 1px solid rgba(0,0,0,0.06); padding: 4px 12px; border-radius: 99px;">
            <span style="color: #15803d;" title="{i_sehat} Healthy Branches">✓ {i_sehat}</span>
            <span style="color: #b45309;" title="{i_waspada} Warning Branches">! {i_waspada}</span>
            <span style="color: #b91c1c;" title="{i_kritis} Critical Branches">x {i_kritis}</span>
          </div>
        {/if}
        {#if d.id === 'Member'}
          <div style="position: absolute; top: 16px; right: 16px; display: flex; gap: 10px; font-size: 0.85rem; font-weight: 700; background: rgba(0,0,0,0.04); border: 1px solid rgba(0,0,0,0.06); padding: 4px 12px; border-radius: 99px;">
            <span style="color: #15803d;" title="{m_sehat} Healthy">✓ {m_sehat}</span>
            <span style="color: #b45309;" title="{m_waspada} Warning">! {m_waspada}</span>
            <span style="color: #b91c1c;" title="{m_kritis} Critical">x {m_kritis}</span>
            <span style="color: #0284c7;" title="{m_special} Special">★ {m_special}</span>
          </div>
        {/if}
        {#if d.id === 'Pegawai'}
          <div style="position: absolute; top: 16px; right: 16px; display: flex; gap: 10px; font-size: 0.85rem; font-weight: 700; background: rgba(0,0,0,0.04); border: 1px solid rgba(0,0,0,0.06); padding: 4px 12px; border-radius: 99px;">
            <span style="color: #15803d;" title="{p_sehat} Healthy Indicators">✓ {p_sehat}</span>
            <span style="color: #b45309;" title="{p_waspada} Warning Indicators">! {p_waspada}</span>
            <span style="color: #b91c1c;" title="{p_kritis} Critical Indicators">x {p_kritis}</span>
          </div>
        {/if}
        {#if d.id === 'Menu'}
          <div style="position: absolute; top: 16px; right: 16px; display: flex; gap: 10px; font-size: 0.85rem; font-weight: 700; background: rgba(0,0,0,0.04); border: 1px solid rgba(0,0,0,0.06); padding: 4px 12px; border-radius: 99px;">
            <span style="color: #15803d;" title="{mnu_stable} Active/Stable Menus">▲ {mnu_stable}</span>
            <span style="color: #b91c1c;" title="{mnu_declining} Dropping Menus">▼ {mnu_declining}</span>
            <span style="color: #475569;" title="{mnu_passive} Passive Menus (Food Waste Risk)">■ {mnu_passive}</span>
          </div>
        {/if}
        {#if d.id === 'Jam Sibuk'}
          <div style="position: absolute; top: 16px; right: 16px; display: flex; gap: 10px; font-size: 0.85rem; font-weight: 700; background: rgba(0,0,0,0.04); border: 1px solid rgba(0,0,0,0.06); padding: 4px 12px; border-radius: 99px;">
            <span style="color: #b91c1c;" title="{tf_liar} Dynamic/Wild Branches">≈ {tf_liar}</span>
            <span style="color: #b45309;" title="{tf_terkendali} Controlled Branches">≃ {tf_terkendali}</span>
            <span style="color: #15803d;" title="{tf_stabil} Very Stable Branches">= {tf_stabil}</span>
          </div>
        {/if}
      </a>
    {/each}
...
{:else}
  <GlobalLoading title="Preparing Main Dashboard..." desc="Synchronizing all restaurant operational metrics. Please wait a moment." />
{/if}
