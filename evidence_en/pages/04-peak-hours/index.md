...
<script>
  ...
  function formatPeak(phl, bd) {
    if (!phl || !bd) return '';
    return `${formatTime(phl)} (${translateDay(bd)})`;
  }
</script>
<style>
/* ── Hero ── */
...
<div class="pt-page">
  <div class="hero">
    <div class="hero-eyebrow" style="margin-bottom: 0; font-size: 11px;">⏰ Demand & Traffic · This Month's Snapshot</div>
    <div class="hero-grid">
      
      <!-- Main Block: Volatility (Difficulty) -->
      <div class="hero-main-card {vl === 'tinggi' ? 'status-kritis' : (vl === 'sedang' ? 'status-waspada' : 'status-sehat')}">
        <div class="hero-eyebrow">Demand Characteristics</div>
        <div class="hero-stat-number" style="font-size: 2.2rem; margin-top: 16px; margin-bottom: 8px;">
          {#if vl === 'tinggi'}High Volatility (Unstable)
          {:else if vl === 'sedang'}Moderate Volatility (Controlled)
          {:else}Low Volatility (High Stability){/if}

        </div>
        <div class="hero-stat-label" style="text-transform: none; font-size: 1.15rem; margin-bottom: 0;">Mean Daily Deviation: <strong>±{cv}%</strong></div>
      </div>
      
      <!-- Supporting Blocks -->
      <div class="hero-side">
        
        <!-- Block 1: Peak Consumption Window -->
        <div class="hero-side-card">
          <div class="hero-side-label">⏰ Peak Consumption Window</div>
          <div class="hero-side-value">{formatPeak(phl, bd)}</div>
          <div class="hero-side-note">
            Primary operational throughput bottleneck. Represents <strong>{wsp}%</strong> of total weekly volume.
          </div>
        </div>
        
        <!-- Block 2: Seasonality -->
        <div class="hero-side-card">
          <div class="hero-side-label">🔁 Peak Demand Seasonality</div>
          <div class="hero-side-value">Q{sq} ({hep > 0 ? '+' : ''}{hep}%)</div>
          <div class="hero-side-note">
            Highest annual demand period, reflecting volume surges relative to baseline daily averages.
          </div>
        </div>
  ...
  <details class="guide-acc" style="margin-top: 2px; margin-bottom: 24px;">
    <summary>💡 Demand Volatility Framework &amp; Operational Guidance</summary>
    <div class="guide-body">
      <p style="margin-top: 4px; margin-bottom: 16px; font-weight: 500; color: var(--color-text-secondary);">
        Demand volatility metrics establish the baseline for labor scheduling and prep-sheet buffer sizing. Quantifying daily variance from expected volume dictates required operational flexibility.
      </p>
      <div class="guide-grid" style="grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 12px;">
        <div class="guide-card teal">
          <div class="guide-card-icon">🎯</div>
          <div class="guide-card-content">
            <div class="guide-card-label">Variance &lt; 10%</div>
            <h4 class="guide-card-title">Low Volatility (Predictable)</h4>
            <p class="guide-card-desc">Daily volume remains highly consistent. Fixed labor schedules and lean inventory buffers can be deployed with minimal operational risk.</p>
          </div>
        </div>
        <div class="guide-card orange">
          <div class="guide-card-icon">🌊</div>
          <div class="guide-card-content">
            <div class="guide-card-label">Variance 10% – 20%</div>
            <h4 class="guide-card-title">Moderate Volatility (Controlled)</h4>
            <p class="guide-card-desc">Volume experiences moderate shifts driven by external factors. Baseline scheduling remains effective with minor safety stock and shift buffers.</p>
          </div>
        </div>
        <div class="guide-card purple">
          <div class="guide-card-icon">🎢</div>
          <div class="guide-card-content">
            <div class="guide-card-label">Variance &gt; 20%</div>
            <h4 class="guide-card-title">High Volatility (Unstable)</h4>
            <p class="guide-card-desc">Demand exhibits high fluctuation with stockout and labor under-allocation risks. Requires flexible labor rostering and elevated safety stock levels.</p>
          </div>
        </div>
        <div class="guide-card blue">
          <div class="guide-card-icon">🔭</div>
          <div class="guide-card-content">
            <div class="guide-card-label">Diagnostic Protocol</div>
            <h4 class="guide-card-title">Anomaly Investigation</h4>
            <p class="guide-card-desc">For branches flagged with High Volatility, review the <strong>Peak Volume Analytics</strong> module to isolate specific day-of-week variance drivers.</p>
          </div>
        </div>
      </div>
      <div style="font-size: 0.75rem; color: var(--color-text-secondary); margin-top: 12px;">
        *Mean Absolute Deviation (MAD) formula: Average absolute difference between actual daily demand volume and projected baseline figures.
      </div>
    </div>
  </details>
  ...
  <div style="display: flex; align-items: center; gap: 8px; margin-bottom: 24px; margin-top: 16px;">
    ...
    <div>
      <h2 style="margin: 0; font-size: 1.25rem; font-weight: 800; color: var(--color-text-primary); letter-spacing: -0.02em; text-transform: uppercase;">
        BRANCH DEMAND PROFILE &amp; VOLATILITY LEDGER
      </h2>
      <div style="font-size: 0.85rem; color: var(--color-text-tertiary); font-weight: 500;">
        Branch-level operational traffic stability, peak throughput windows, and demand volatility metrics (Trailing 30 Days).
      </div>
    </div>
  </div>
    ...
    <div class="branch-health-card {branchStatusClass}">
      ...
      <div class="branch-card-header">
        ...
        <span class="branch-status-badge {branchStatusClass}">
          {row.volatilitas > 20 ? '🎢 High Volatility' : row.volatilitas > 10 ? '🌊 Moderate Volatility' : '🎯 Low Volatility'}
        </span>
      </div>
      ...
      <div class="branch-margin-section" style="grid-template-columns: 1fr; text-align: center; padding: 12px 0;">
        <div class="branch-margin-active-box" style="align-items: center;">
          <div class="branch-margin-main {branchStatusClass}">{formatTime(row.weekday_peak)}</div>
          <div class="branch-margin-label">Combined Peak Window</div>
        </div>
      </div>
      {:else}
      <div class="branch-margin-section" style="grid-template-columns: 1fr 1px 1fr; gap: 0; text-align: center; padding: 12px 0;">
        <div class="branch-margin-active-box" style="align-items: center; padding-right: 12px;">
          <div class="branch-margin-main {branchStatusClass}">{formatTime(row.weekday_peak)}</div>
          <div class="branch-margin-label">Weekday Peak Window</div>
        </div>
        ...
        <div class="branch-margin-active-box" style="align-items: center; padding-left: 12px;">
          <div class="branch-margin-main {branchStatusClass}">{formatTime(row.weekend_peak)}</div>
          <div class="branch-margin-label">Weekend Peak Window</div>
        </div>
      </div>
      {/if}
      ...
      <div class="branch-stats-grid">
        <div class="stat-pill">
          ...
          <span class="stat-value">{translateDay(row.hari_puncak)}</span>
        </div>
        <div class="stat-pill">
          <span class="stat-label">Peak Qtr</span>
          <span class="stat-value">Q{row.puncak_musiman}</span>
        </div>
        <div class="stat-pill">
          <span class="stat-label">MAD</span>
          <span class="stat-value">±{row.volatilitas}%</span>
        </div>
      </div>
