...
<script>
  ...
  function formatTime(phl) {
    if (!phl) return '';
    return phl.replace(/\b(\d{1,2}):\d{2}\b/g, (match, hourStr) => {
  ...
  function translateDay(bd) {
    if (!bd) return '';
    if (bd === 'Semua Days' || bd.includes('SELECT NULL')) return 'All Days';
    const days = {
      'Senin': 'Monday',
      'Selasa': 'Tuesday',
      'Rabu': 'Wednesday',
...
{#if branch_list.length > 0}
  {@const branchValue = String(inputs.pilih_location?.value ?? inputs.pilih_location ?? '')}
  ...
  <div style="margin-top: 24px; margin-bottom: 24px;">
    <SectionCard 
      eyebrow="<span style='font-size: 12px;'>🏪 LOCATION SELECTOR</span>" 
      title="Branch Throughput &amp; Peak Window Control" 
      description="Select a specific branch to scope traffic density analysis, hourly peak operating windows, and weekend volume trends."
    >
      <ButtonGroup name="pilih_location">
  ...
  <Tabs fullWidth=true>
    <Tab label="⏰ Peak Throughput Hours">
  ...
  {@const badgeText = ratio > 1.4 ? '🔥 Akhir Pekan Ekstrem' : ratio > 1.1 ? '🎉 Cenderung Akhir Pekan' : ratio > 0.9 ? '⚖️ Merata Stabil' : ratio > 0.7 ? '👔 Cenderung Days Kerja' : '🏢 Days Kerja Ekstrem'}
  ...
  {@const badgeDesc = ratio > 1.4 ? 'Akhir pekan mendominasi telak. Maksimalkan stock Jumat-Minggu, tekan biaya operasional hari kerja.' : ratio > 1.1 ? 'Lebih ramai di akhir pekan. Geser jadwal libur staf inti ke awal minggu (Senin/Selasa).' : ratio > 0.9 ? 'Volume pesanan sangat konsisten tiap hari. Terapkan jadwal shift dan stock yang merata.' : ratio > 0.7 ? 'Ditopang pekerja kantoran. Jam makan siang krusial, siapkan promo keluarga di akhir pekan.' : 'Sangat bergantung jam kantor. Tekan drastis jumlah staf saat akhir pekan untuk efisiensi.'}
...
<SectionHeader 
  eyebrow="📈 Diurnal Demand Distribution"
  title="Intraday Operating Velocity & Shift Patterns"
  description="Maps intraday consumption curves from opening to closing. Evaluates peak hour shifts between weekday baselines and weekend demand surges."
/>
...
{#if activeBranchData && activeBranchData.peak_share_pct !== null}
  ...
  <div class="period-strip" style="grid-template-columns: repeat(3, minmax(0, 1fr)); margin-bottom: 32px;">
    ...
    <div class="period-pill {shareStatus}">
      ...
      <div class="period-pill-copy" style="font-size: 0.76rem; line-height: 1.45;">
        Share of total daily transaction volume concentrated within peak operating hours.
      </div>
    </div>
    ...
    <div class="period-pill {surgeStatus}">
      ...
      <div class="period-pill-copy" style="font-size: 0.76rem; line-height: 1.45;">
        Ratio of peak-hour transaction volume compared to off-peak daily baseline averages.
      </div>
    </div>
    
    <div class="period-pill">
      <div class="period-pill-label">🕐 Critical Throughput Windows</div>
      <div class="period-pill-value" style="font-size: 1.05rem;">
        {formatTime(windows)}
      </div>
      <div class="period-pill-copy" style="font-size: 0.76rem; line-height: 1.45;">
        High-demand windows requiring maximum labor capacity and BOH station readiness.
      </div>
    </div>
  ...
  <div style="margin-top: 32px;">
    <Tabs fullWidth=true>
      <Tab label="📈 Weekday vs. Weekend Hourly Variance">
        <div style="padding-top: 8px;">
          <div class="section-head tight" style="margin-bottom: 20px;">
          <div>
            <div class="section-eyebrow">📈 INTRADAY DEMAND CURVES</div>
            <h3 class="section-title">Hourly Order Volume Profile &amp; Day-Type Comparison</h3>
            <p class="section-copy">Compares operating hour demand distribution (08:00–22:00). Dashed Blue Line represents Weekday baseline (Mon–Thu), while Solid Amber Line highlights Weekend volume surges (Fri–Sun).</p>
          </div>
        </div>
          ...
          <ECharts config={{
            tooltip: {
              ...
              formatter: function(params) {
                ...
                let html = "<div style='font-weight:700;margin-bottom:6px;border-bottom:1px solid rgba(0,0,0,0.1);padding-bottom:4px;'>⏰ Operating Window: " + hour + "</div>";
                params.forEach(function(item) {
                  const isWeekend = ['Fri', 'Sat', 'Sun'].includes(item.seriesName);
                  const icon = isWeekend ? '🟧' : '🟦';
                  const val = Math.round(item.value[1]).toLocaleString('en-US');
                  html += "<div style='display:flex;justify-content:space-between;gap:16px;font-size:12px;margin:3px 0;'>" +
                    ...
                    "<span style='font-weight:700;'>" + val + " orders</span></div>";
                });
                return html;
              }
            },
            ...
            yAxis: { type: 'value', name: 'Order Volume' },
            color: ['#93c5fd', '#60a5fa', '#3b82f6', '#1d4ed8', '#fbbf24', '#f97316', '#dc2626'],
            series: [
              { dayName: 'Mon', rawDay: 'Senin' },
              { dayName: 'Tue', rawDay: 'Selasa' },
              { dayName: 'Wed', rawDay: 'Rabu' },
              { dayName: 'Thu', rawDay: 'Kamis' },
              { dayName: 'Fri', rawDay: 'Jumat' },
              { dayName: 'Sat', rawDay: 'Sabtu' },
              { dayName: 'Sun', rawDay: 'Minggu' }
            ].map(function(d) {
              const isWknd = ['Fri', 'Sat', 'Sun'].includes(d.dayName);
              // Mendukung pemetaan dari data backend baik jika field 'hari' bernilai bahasa Indonesia maupun English
              const dayRows = branchHeatmap.filter(function(r) { return r.hari === d.rawDay || r.hari === d.dayName; });
      ...
      <Tab label="📊 Hourly Order Channel Mix">
        <div style="padding-top: 8px;">
          <div class="section-head tight" style="margin-bottom: 16px;">
          <div>
            <div class="section-eyebrow">📊 INTRADAY CHANNEL MIX DISTRIBUTION</div>
            <h3 class="section-title">Hourly Order Type Proportion (Dine-In vs. Takeaway vs. Delivery)</h3>
            <p class="section-copy">Filters order channel breakdown across operating hours (08:00–22:00). Informs front-of-house table capacity management and packaging inventory staging.</p>
          </div>
        </div>

        <div style="margin-bottom: 20px;">
          <ButtonGroup name="pilih_hari_breakdown">
            <ButtonGroupItem value="Semua Days" valueLabel="🌐 All Operating Days" default />
            <ButtonGroupItem value="Senin" valueLabel="Mon" />
            <ButtonGroupItem value="Selasa" valueLabel="Tue" />
            <ButtonGroupItem value="Rabu" valueLabel="Wed" />
            <ButtonGroupItem value="Kamis" valueLabel="Thu" />
            <ButtonGroupItem value="Jumat" valueLabel="Fri" />
            <ButtonGroupItem value="Sabtu" valueLabel="Sat" />
            <ButtonGroupItem value="Minggu" valueLabel="Sun" />
          </ButtonGroup>
        </div>

        {#if true}
        {@const selectedDays = String(inputs.pilih_hari_breakdown?.value ?? inputs.pilih_hari_breakdown ?? 'Semua Days')}
        {@const branchBreakdownData = hourly_ordertype_breakdown.filter(r => r.branch_name === selectedBranch)}
        {@const activeDayData = selectedDays === 'Semua Days' || selectedDays.includes('SELECT NULL') ? branchBreakdownData : branchBreakdownData.filter(r => r.hari === selectedDays)}
        ...
        <div class="period-strip" style="grid-template-columns: repeat(3, minmax(0, 1fr)); margin-bottom: 24px;">
          <div class="period-pill sehat">
            ...
            <div class="period-pill-copy" style="font-size: 0.76rem; line-height: 1.45;">
              {tDineIn.toLocaleString('en-US')} total orders
            </div>
          </div>
          ...
          <div class="period-pill waspada">
            ...
            <div class="period-pill-copy" style="font-size: 0.76rem; line-height: 1.45;">
              {tTakeaway.toLocaleString('en-US')} total orders
            </div>
          </div>
          ...
          <div class="period-pill kritis">
            ...
            <div class="period-pill-copy" style="font-size: 0.76rem; line-height: 1.45;">
              {tDelivery.toLocaleString('en-US')} total orders
            </div>
          </div>
        </div>
          ...
          <ECharts config={{
            tooltip: {
              ...
              formatter: function(params) {
                ...
                let html = "<div style='font-weight:700;margin-bottom:6px;border-bottom:1px solid rgba(0,0,0,0.1);padding-bottom:4px;'>⏰ Time: " + hour + " — " + translateDay(selectedDays) + "</div>";
                params.forEach(function(p) {
                  ...
                  html += "<div style='display:flex;justify-content:space-between;gap:16px;font-size:12px;margin:3px 0;'>" +
                    ...
                    "<span style='font-weight:700;'>" + Math.round(val).toLocaleString('en-US') + " orders (" + pct + "%)</span></div>";
                });
                return html;
              }
            },
        ...
        <div class="chart-insight-bar" style="margin-top: -12px; margin-bottom: 8px;">
          📌 <strong>Channel Mix Insight ({translateDay(selectedDays)}):</strong> An expanding Takeaway (Green) and Online Delivery (Orange) share indicates an off-premise demand shift. Align packaging inventory staging and dedicated dispatch station capacity accordingly.
        </div>
        {/if}
        </div>
      </Tab>
...
{:else}
  <div style="padding: 60px 20px; text-align: center; background: var(--color-background-secondary); border: 1px dashed var(--color-border-tertiary); border-radius: 12px; margin-top: 24px;">
    <h3 style="margin: 0 0 8px 0; color: var(--color-text-primary);">No Operational Data Available</h3>
    <p style="color: var(--color-text-secondary); margin: 0;">
      Please select at least one valid branch location from the filter control above to view throughput analytics.
    </p>
  </div>
{/if}
        {/if}
    ...
    <Tab label="📅 Peak Demand Days">
      <div class="tab-content-wrapper">
        ...
        {@const avgWeekend = totalWeekend / 3}
        {@const ratio = avgWeekday > 0 ? avgWeekend / avgWeekday : 99}
        {@const badgeText = ratio > 1.4 ? '🔥 Extreme Weekend Surge' : ratio > 1.1 ? '🎉 Weekend-Skewed' : ratio > 0.9 ? '⚖️ Balanced Daily Volume' : ratio > 0.7 ? '👔 Weekday-Skewed' : '🏢 Extreme Weekday Peak'}
        {@const badgeBg = ratio > 1.4 ? '#fee2e2' : ratio > 1.1 ? '#fef3c7' : ratio > 0.9 ? '#dcfce7' : ratio > 0.7 ? '#eff6ff' : '#f3f4f6'}
        {@const badgeBorder = ratio > 1.4 ? '#fca5a5' : ratio > 1.1 ? '#fcd34d' : ratio > 0.9 ? '#86efac' : ratio > 0.7 ? '#bfdbfe' : '#d1d5db'}
        {@const badgeColor = ratio > 1.4 ? '#991b1b' : ratio > 1.1 ? '#92400e' : ratio > 0.9 ? '#166534' : ratio > 0.7 ? '#1e40af' : '#374151'}
        {@const badgeDesc = ratio > 1.4 
          ? 'Significant volume concentration on Fri–Sun. Operational focus should prioritize weekend shift coverage and stock availability.' 
          : ratio > 1.1 
          ? 'Higher order volume observed on weekends. Recommends aligning core staffing levels with peak Friday–Sunday demand.' 
          : ratio > 0.9 
          ? 'Order volume remains consistent across all operating days. Suitable for standardized weekly labor and inventory schedules.' 
          : ratio > 0.7 
          ? 'Higher transaction volume concentrated on weekdays (Mon–Thu). Highlights weekday meal periods as primary revenue drivers.' 
          : 'Demand is predominantly weekday-driven. Indicates lower weekend throughput relative to weekday operating baselines.'}
  ...
  <div style="margin-top: 32px; margin-bottom: 32px;">
        ...
        <div style="display: flex; flex-direction: column; align-items: flex-start; justify-content: flex-start; margin-top: 32px; margin-bottom: 24px; gap: 16px; text-align: left;">
          <div style="width: 100%;">
            <div class="section-head tight" style="margin-bottom: 0px;">
              <div>
                <div class="section-eyebrow">🧬 DAY-TYPE DEMAND PROFILE (TRAILING 30 DAYS)</div>
                <h3 class="section-title">Weekday vs. Weekend Volume Distribution</h3>
                <p class="section-copy">Evaluates order volume distribution between weekday baselines and weekend operational traffic.</p>
              </div>
            </div>
          </div>
          ...
          <div style="display: flex; flex-direction: column; align-items: center; justify-content: center; width: 100%; text-align: center; margin-top: 16px;">
            ...
            <div style="width: 100%; max-width: 600px; height: 320px; margin-top: 24px;">
              <ECharts config={{
                tooltip: { 
                  trigger: 'item',
                  formatter: function(p) {
                    return p.name + '<br/><b>' + p.value.toLocaleString('en-US') + ' orders</b> (' + p.percent + '%)';
                  }
                },
                color: ['#3b82f6', '#f59e0b'],
                legend: { show: false },
                series: [
                  {
                    ...
                    data: branchWeekendData.map(row => ({ 
                      value: row.total_orders, 
                      name: row.period_type.includes('Weekday') ? 'Weekday (Mon-Thu)' : 'Weekend (Fri-Sun)' 
                    })),
          ...
          <div class="chart-insight-bar" style="margin-top: 32px; width: 100%; text-align: left;">
            📌 <strong>Operational Directive (Branch Profile Baseline):</strong> Utilize this branch ratio as an operational baseline to structure location-specific promotional campaigns, inventory safety stock buffers, and core labor scheduling.
          </div>
        </div>
      </div>
        ...
        <div class="section-head tight" style="margin-top: 40px; margin-bottom: 12px;">
          <div>
            <div class="section-eyebrow">📊 AGGREGATED DAILY VOLUME (TRAILING 30 DAYS)</div>
            <h3 class="section-title">Daily Order Volume &amp; Kitchen Throughput Peaks</h3>
            <p class="section-copy">Identifies historical day-of-week demand patterns to highlight peak operational throughput requirements.</p>
          </div>
        </div>
        <BarChart 
          data={branchDayData.map(d => ({ ...d, hari: translateDay(d.hari) }))} 
          x="hari" 
          y="total_orders"
          series="order_type"
          type="stacked"
          sort={false}
          yAxisTitle="Order Volume (Trailing 30 Days)" 
          xAxisTitle="Day of Week" 
        />
        <div class="chart-insight-bar" style="margin-top: 16px;">
          📌 <strong>Operational Directive (Macro Demand):</strong> Schedule high-volume inventory prep shifts 24 hours prior to identified peak demand days. Evaluate off-peak day capacity to optimize prep-sheet distribution and labor utilization across the operating week.
        </div>
        ...
        {:else}
          <div style="padding: 60px 20px; text-align: center; background: var(--color-background-secondary); border: 1px dashed var(--color-border-tertiary); border-radius: 12px; margin-top: 24px;">
            <h3 style="margin: 0 0 8px 0; color: var(--color-text-primary);">No Operational Data Available</h3>
            <p style="color: var(--color-text-secondary); margin: 0;">Please select at least one valid branch location from the filter control above to view seasonality analytics.</p>
          </div>
        {/if}
      </div>
    </Tab>

    <Tab label="🔁 Quarterly Seasonality">
      <div class="tab-content-wrapper">
        {#if true}
        {@const selectedBranch = inputs.pilih_location?.value ?? inputs.pilih_location}
  ...
  <div class="warning-banner">
    ...
    <div>
      <div class="warning-banner-title">Data Integrity Notice: Partial Month Exclusion</div>
      <div class="warning-banner-desc">
        To preserve baseline integrity, all quarterly aggregates and seasonal trend models exclusively evaluate fully completed calendar months. In-progress month data is strictly <strong>excluded</strong> to prevent partial-period bias from distorting analytical outputs.
      </div>
    </div>
  </div>
...
{#if isDataSufficient}
  ...
  <div style="margin-top: 0px; margin-bottom: 8px;">
    <details class="guide-acc" style="margin-bottom:0px;">
      <summary>💡 Seasonality Framework &amp; Quarterly Breakdown</summary>
      <div class="guide-body" style="padding: 16px;">
        <p style="margin-top: 0px; margin-bottom: 20px; font-weight: 500; color: var(--color-text-secondary); line-height: 1.6;">
          Tracks multi-year demand seasonality to identify high-volume quarters and recurring demand shifts. Establishes long-term baselines for annual labor budgeting and procurement planning.
        </p>
        <div style="font-size:10px;font-weight:700;letter-spacing:0.1em;text-transform:uppercase;color:var(--color-text-tertiary);margin-bottom:12px;">🗓️ QUARTERLY SCHEDULE</div>
        <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:12px;margin-bottom:24px;">
          <div class="q-card q1">
            <div style="font-size:1.1rem;font-weight:900;color:#4338ca;margin-bottom:4px;">Q1</div>
            <div style="font-size:0.9rem;font-weight:700;color:var(--color-text-primary);margin-bottom:6px;">Jan – Mar</div>
            <div style="font-size:0.8rem;color:var(--color-text-secondary);line-height:1.5;">Q1 Baseline: Captures post-holiday demand normalization and early Q1 operating trends.</div>
          </div>
          <div class="q-card q2">
            <div style="font-size:1.1rem;font-weight:900;color:#0f766e;margin-bottom:4px;">Q2</div>
            <div style="font-size:0.9rem;font-weight:700;color:var(--color-text-primary);margin-bottom:6px;">Apr – Jun</div>
            <div style="font-size:0.8rem;color:var(--color-text-secondary);line-height:1.5;">Q2 Transition: Reflects mid-year demand volume and early mid-year promotional surges.</div>
          </div>
          <div class="q-card q3">
            <div style="font-size:1.1rem;font-weight:900;color:#b45309;margin-bottom:4px;">Q3</div>
            <div style="font-size:0.9rem;font-weight:700;color:var(--color-text-primary);margin-bottom:6px;">Jul – Sep</div>
            <div style="font-size:0.8rem;color:var(--color-text-secondary);line-height:1.5;">Q3 Stabilization: Measures post-summer volume shifts and Q3 operational baselines.</div>
          </div>
          <div class="q-card q4">
            <div style="font-size:1.1rem;font-weight:900;color:#b91c1c;margin-bottom:4px;">Q4</div>
            <div style="font-size:0.9rem;font-weight:700;color:var(--color-text-primary);margin-bottom:6px;">Oct – Dec</div>
            <div style="font-size:0.8rem;color:var(--color-text-secondary);line-height:1.5;">Q4 Year-End Surge: Tracks holiday season volume concentration and year-end demand acceleration.</div>
          </div>
        </div>
        <div style="font-size:10px;font-weight:700;letter-spacing:0.1em;text-transform:uppercase;color:var(--color-text-tertiary);margin-bottom:12px;">📊 CALCULATION METHODOLOGY</div>
        <div class="support-grid">
          <div class="support-item">
            <div class="support-item-label">Seasonal Variance</div>
            <div class="support-item-title">Peak vs. Trough Quarterly Spread</div>
            <div class="support-item-desc">Quantifies volume variance between peak and off-peak quarters. High variance dictates dynamic annual resource allocation; low variance allows uniform baselines.</div>
          </div>
          <div class="support-item">
            <div class="support-item-label">Holiday Impact</div>
            <div class="support-item-title">Holiday vs. Baseline Months</div>
            <div class="support-item-desc">Measures order volume shifts during major holiday periods relative to standard operating calendar months.</div>
          </div>
          <div class="support-item">
            <div class="support-item-label">Normalization</div>
            <div class="support-item-title">Monthly Average Per Quarter</div>
            <div class="support-item-desc">Calculated using average monthly order volume per quarter to prevent partial-year data gaps from skewing seasonal trends.</div>
          </div>
        </div>
      </div>
    </details>
...
<SectionHeader 
  eyebrow="🗓️ SEASONAL & HOLIDAY IMPACT ANALYTICS"
  title="Quarterly Demand Variance & Holiday Volume Lift"
  description="Evaluates peak quarterly volume, holiday demand lifts, and operational baseline variances across the annual calendar."
/>
...
<div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 12px; margin-bottom: 32px;">
  <div class="kpi-card strong">
    ...
    <div class="kpi-interp">
      Quarter with the highest average monthly order volume within the observed period.
    </div>
  </div>
  <div class="kpi-card growth">
    <div class="kpi-label">📏 SEASONAL SPREAD</div>
    <div class="kpi-number">{sgp}%</div>
    <div class="kpi-interp">
      Percentage difference in monthly order volume between peak ({sq}) and trough ({wq}) quarters.
      {#if sgp >= 40} Indicates high seasonal volume variance across quarters.
      {:else if sgp >= 20} Indicates moderate seasonal volume variance across quarters.
      {:else} Indicates baseline order volume stability across all quarters.{/if}
    </div>
  </div>
</div>
	  ...
	  <div class="strategic-stack">
            ...
            <div style="display: flex; gap: 16px; align-items: center; justify-content: center; margin-bottom: 24px;">
              ...
              <div style="flex: 1; padding: 24px; border-radius: 16px; background: linear-gradient(135deg, rgba(245,158,11,0.1) 0%, rgba(245,158,11,0.02) 100%); border: 1px solid rgba(245,158,11,0.2); text-align: center; position: relative; transition: all 0.3s ease; cursor: default;" onmouseover="this.style.transform='translateY(-4px)'; this.style.boxShadow='0 10px 15px -3px rgba(245,158,11,0.2)'" onmouseout="this.style.transform='none'; this.style.boxShadow='none'">
                ...
                <div style="font-size: 0.9rem; font-weight: 700; color: #b45309; text-transform: uppercase; letter-spacing: 0.05em; margin-bottom: 4px;">HOLIDAY &amp; PEAK SEASONS</div>
                <div style="font-size: 0.8rem; color: var(--color-text-tertiary); margin-bottom: 16px;">July, December &amp; Ramadan / Eid Periods</div>
                <div style="font-size: 2rem; font-weight: 800; color: var(--color-text-primary);">{#if ho}{Math.round(ho).toLocaleString('en-US')}{:else}0{/if}</div>
                <div style="font-size: 0.9rem; color: var(--color-text-secondary); margin-top: 4px;">Monthly Order Average</div>
              </div>
              ...
              <div style="display: flex; flex-direction: column; align-items: center; justify-content: center; z-index: 10;">
                ...
                <div style="font-size: 0.75rem; color: var(--color-text-tertiary); margin-top: 8px; font-weight: 600; text-transform: uppercase;">Volume Variance</div>
              </div>
              ...
              <div style="flex: 1; padding: 24px; border-radius: 16px; background: rgba(0,0,0,0.02); border: 1px solid rgba(0,0,0,0.08); text-align: center; transition: all 0.3s ease; cursor: default;" onmouseover="this.style.transform='translateY(-4px)'; this.style.boxShadow='0 10px 15px -3px rgba(0,0,0,0.1)'" onmouseout="this.style.transform='none'; this.style.boxShadow='none'">
                <div style="font-size: 2.5rem; margin-bottom: 12px;">📅</div>
                <div style="font-size: 0.9rem; font-weight: 700; color: var(--color-text-secondary); text-transform: uppercase; letter-spacing: 0.05em; margin-bottom: 4px;">REGULAR OPERATING MONTHS</div>
                <div style="font-size: 0.8rem; color: var(--color-text-tertiary); margin-bottom: 16px;">Standard Operating Periods</div>
                <div style="font-size: 2rem; font-weight: 800; color: var(--color-text-primary);">{#if ro}{Math.round(ro).toLocaleString('en-US')}{:else}0{/if}</div>
                <div style="font-size: 0.9rem; color: var(--color-text-secondary); margin-top: 4px;">Monthly Order Average</div>
              </div>
            </div>
            <div class="chart-interp" style="margin-top: 16px;">
              📌 <strong>Executive Summary:</strong> During designated peak operating periods (School Holidays, Year-End, and Ramadan/Eid), order volume reflects a <strong>{#if hep > 0}+{hep}%{:else}{hep}%{/if}</strong> variance relative to standard baseline months.
            </div>
    ...
    <div class="section-head tight" style="margin-bottom: 16px;">
      <div>
        <div class="section-eyebrow">📊 HISTORICAL QUARTERLY DISTRIBUTION</div>
        <div class="section-title" style="font-size: 1.3rem; font-weight: 800; margin: 4px 0;">Multi-Year Quarterly Trend Formation</div>
        <div class="section-copy">Historical order volume baseline data illustrating quarterly demand shifts across annual operating cycles.</div>
      </div>
    </div>

    <BarChart data={activeQuarterly.sort((a,b) => a.kuartal.localeCompare(b.kuartal))} x=kuartal y=q_orders series=tahun type="grouped" sort=false title="Total Orders per Quarter — by Year" xAxisTitle="Quarter" yAxisTitle="Total Orders" yFmt='#,##0 "Orders"' />
    
    <PremiumTable 
      data={activeQuarterlyAvg} 
      pageSize={4} 
      columns={[
        { title: "Quarter", key: "kuartal", align: "center", bold: true },
        { title: "Avg Orders", key: "avg_q_orders", align: "right", type: "currency_raw" },
        { title: "Order Share", key: "pct_orders", align: "right", type: "pct_ratio" },
        { title: "Avg Revenue", key: "avg_q_revenue", align: "right", type: "currency_raw" },
        { title: "Revenue Share", key: "pct_revenue", align: "right", type: "pct_ratio" }
      ]} 
    />

    <div class="chart-interp" style="margin-top: 16px;">
      📌 <strong>Multi-Year Trend Validation:</strong> Evaluates recurring year-over-year volume consistency across quarters to distinguish multi-year seasonal patterns from single-year operational anomalies.
    </div>
