...
<div class="hero" style="margin-bottom: 32px; margin-top: 10px;">
  <div class="hero-eyebrow">💎 Member Insights (Last 30 Days) · <Value data={member_dates} column="tgl_30_awal"/> - <Value data={member_dates} column="tgl_akhir"/></div>
  <div class="hero-grid">
    <div class="hero-main-card {heroStatusClass}">
      <div class="hero-stat-number">{usFormat(activeRate, 1)}%</div>
      <div class="hero-stat-label">ACTIVE MEMBER PENETRATION (30D)</div>
      <div class="hero-subtitle">
        Out of <strong>{usFormat(totalMembers)} total registered profiles</strong> evaluated across the trailing 30-day operating window.
      </div>
    </div>
    <div class="hero-side">
      <div class="hero-side-card">
        <div class="hero-side-label">🚨 Pipeline Churn Exposure</div>
        <div class="hero-side-value">-Rp <Value data={retention_stats} column="potential_loss" fmt="#,##0"/></div>
        <div class="hero-side-note">Projected gross revenue at risk across {usFormat(totalChurn)} dormant or degrading member profiles.</div>
      </div>
      <div class="hero-side-card">
        <div class="hero-side-label">🌱 Repeat Activation Velocity</div>
        <div class="hero-side-value">{activationRatePct > 0 ? usFormat(activationRatePct, 1) + '%' : '0%'}</div>
        <div class="hero-side-note">Secondary transaction conversion rate for newly acquired profiles within the 30-day window.</div>
      </div>
    </div>
  </div>
</div>
...
<div style="margin-top: 32px; margin-bottom: 20px; display: flex; align-items: center; gap: 10px;">
  ...
  <div>
    ...
    <div style="font-size: 0.85rem; color: var(--color-text-tertiary); font-weight: 500;">Algorithmic retention workflows and immediate execution priorities for front-of-house (FOH) management.</div>
  </div>
</div>
...
<div class="menu-health">
  <div class="menu-health-head">
    ...
    <div class="menu-health-badges">
      ...
      <span class="menu-health-badge special" style="background: rgba(2,132,199,0.06); border: 1px solid rgba(2,132,199,0.15); color: #0284c7;">★ {milestone_count.length > 0 && milestone_count[0].count > 0 ? 1 : 0} special</span>
    </div>
  </div>
  <div class="menu-health-list">
    <a href="/06-member-behavior/02-tactical-action#ringkasan-risiko" style="text-decoration: none; color: inherit;">
      <div class="menu-health-row {churnRiskState}">
        <div class="menu-health-icon">{churnRiskState === 'safe' ? '✅' : churnRiskState === 'warn' ? '⚠️' : '🚨'}</div>
        <div style="width: 100%;">
          <span class="menu-health-title">Member Churn Rate Exposure</span> 
          <span class="menu-health-copy">- <span class="menu-health-value">{usFormat(churnRatePct, 1)}% ({totalChurn} Members)</span> classified within the churn threshold. Benchmarks: Target = less than 10%, Warning = 10-20%, Critical = greater than 20%.</span>
        </div>
      </div>
    </a>
    <a href="/06-member-behavior/02-tactical-action#milestone" style="text-decoration: none; color: inherit;">
      <div class="menu-health-row special">
        <div class="menu-health-icon">💎</div>
        <div style="width: 100%;">
          <span class="menu-health-title">Loyalty Milestone Recognition</span> 
          <span class="menu-health-copy">- <span class="menu-health-value">{milestone_count.length > 0 ? milestone_count[0].count : 0} Members</span> statistically approaching visit milestone thresholds (10th, 25th, or 50th transaction). Eligible for targeted FOH engagement or algorithmic reward deployment.</span>
        </div>
      </div>
    </a>
  </div>
...
<details class="guide-acc" style="margin-top: 16px; margin-bottom: 24px;">
  <summary>💡 Methodological Context: Churn &amp; Velocity Benchmarking</summary>
  <div class="guide-body" style="padding: 16px;">
    <p style="margin-top: 4px; margin-bottom: 16px; font-weight: 500; color: var(--color-text-secondary);">
      These core metrics serve as leading indicators for cohort retention health, enabling proactive FOH intervention prior to structural revenue contraction.
    </p>
    <div class="guide-grid" style="grid-template-columns: repeat(auto-fit, minmax(280px, 1fr)); gap: 16px;">
      <div class="guide-card blue">
        ...
        <div class="guide-card-content">
          <div class="guide-card-label">Member Churn Rate Exposure</div>
          <h4 class="guide-card-title">Algorithmic Defection Detection</h4>
          <p class="guide-card-desc">Retaining established high-LTV cohorts requires exponentially lower acquisition cost (CAC) than net-new top-of-funnel generation. This metric isolates velocity degradation to optimize win-back workflows.</p>
        </div>
      </div>
      <div class="guide-card blue">
        <div class="guide-card-icon">💎</div>
        <div class="guide-card-content">
          <div class="guide-card-label">Loyalty Milestone Recognition</div>
          <h4 class="guide-card-title">Lifecycle Milestone Engagement</h4>
          <p class="guide-card-desc">Acknowledging hyper-specific visit milestones (e.g., 10th, 25th, or 50th transaction) algorithmically reinforces brand affinity and extends the baseline LTV trajectory.</p>
        </div>
      </div>
    </div>
  </div>
...
<div style="margin-bottom: 16px; display: flex; align-items: center; gap: 10px; margin-top: 48px; padding-top: 32px; border-top: 2px dashed rgba(0,0,0,0.06);">
  ...
  <div>
    ...
    <div style="font-size: 0.85rem; color: var(--color-text-tertiary); font-weight: 500;">Macro-level evaluation of loyalty program contribution and cohort-level unit economics.</div>
  </div>
</div>

<div class="kpi-grid-2" style="margin-bottom: 12px;">
  <div class="kpi-card revenue">
    <div class="kpi-label">💰 Member Average Order Value (AOV)</div>
    <div class="kpi-value">Rp {usFormat(aov)}</div>
    <div class="kpi-meta">
      {#if aovDiffPct > 0}
        <span class="trend-indicator up">▲ {usFormat(aovDiffPct, 1)}%</span>
      {:else if aovDiffPct < 0}
        <span class="trend-indicator down">▼ {usFormat(Math.abs(aovDiffPct), 1)}%</span>
      {:else}
        <span class="trend-indicator neutral">0.0%</span>
      {/if}
    </div>
    <div class="kpi-prev">Non-member benchmark: Rp {usFormat(aovNonMember)}</div>
  </div>
  <div class="kpi-card revenue">
    <div class="kpi-label">💳 Member Gross Revenue</div>
    <div class="kpi-value">Rp {usFormat(totalMemberSpend)}</div>
    <div class="kpi-meta">
      <span style="color: var(--color-text-primary); font-weight: 600; display: inline-flex; align-items: center; gap: 4px;">📊 {usFormat(pctRevenueMember, 1)}%</span>
    </div>
    <div class="kpi-prev">Share of total restaurant gross revenue.</div>
  </div>
</div>
<div class="kpi-grid" style="margin-bottom: 24px;">
  <div class="kpi-card margin">
    <div class="kpi-label">🌱 Member Acquisition Velocity</div>
    <div class="kpi-value">{usFormat(newMembers)}</div>
    <div class="kpi-prev" style="margin-top: 12px;">Net-new enrollments generated top-of-funnel.</div>
  </div>
  <div class="kpi-card margin">
    <div class="kpi-label">📦 Member Order Volume</div>
    <div class="kpi-value">{usFormat(totalMemberOrders)}</div>
    <div class="kpi-meta">
      <span style="color: var(--color-text-primary); font-weight: 600; display: inline-flex; align-items: center; gap: 4px;">📊 {usFormat(pctOrder, 1)}%</span>
    </div>
    <div class="kpi-prev">Share of aggregate organizational ticket volume.</div>
  </div>
  <div class="kpi-card net">
    <div class="kpi-label">🔁 Transaction Frequency Rate</div>
    <div class="kpi-value">{usFormat(totalMemberOrders / activeMembers, 1)}x</div>
    <div class="kpi-meta">
      <span style="color: var(--color-text-primary); font-weight: 600;">Average ticket velocity</span>
    </div>
    <div class="kpi-prev">per active cohort member.</div>
  </div>
</div>
...
<div class="clean-cta-banner">
  <div class="clean-cta-content">
    ...
    <div class="clean-cta-text">
      ...
      <p class="clean-cta-desc">Evaluate promotional campaign ROI, transaction velocity trends, and margin optimization directives across member cohorts.</p>
    </div>
  </div>
  <a href="/06-member-behavior/03-analysis" class="clean-cta-button">
