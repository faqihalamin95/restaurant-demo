...
<h2 class="diagnostics-title">Employee Rankings (Top 5)</h2>
  <p class="diagnostics-copy">Highlight of top revenue-generating personnel, alongside personnel exhibiting elevated overtime utilization risk.</p>
</div>
...
<div class="leaderboards-container">
  ...
  <div class="lb-panel">
    <div class="lb-header green">
      ...
      <div>
        <h3 class="lb-title">Top Revenue Producers</h3>
        <p class="lb-desc">Highest grossing personnel.</p>
      </div>
    </div>
    <div class="lb-body">
  ...
  <div class="lb-panel">
    <div class="lb-header red">
      <div class="lb-header-icon">🔥</div>
      <div>
        <h3 class="lb-title">Utilization Risk</h3>
        <p class="lb-desc">Highest accumulated supplementary hours.</p>
      </div>
    </div>
    <div class="lb-body">
      {#each top_overtime as row, i}
        <div class="lb-row">
          ...
          <div class="lb-score red-text">{row.val} Hours</div>
        </div>
      {/each}
    </div>
  </div>
...
SELECT 
    ...
    MAX(role) as "Role",
    MAX(branch_name) as "Primary Location",
    COUNT(*) as "Total Shifts",
    SUM(overtime_hours) as "Total Overtime (Hours)",
    SUM(CASE WHEN attendance_status IN ('late', 'absent', 'leave') THEN 1 ELSE 0 END) as "Disciplinary Incidents",
    SUM(base_salary) as "Base Salary (Rp)",
    SUM(total_revenue) as "Total Revenue (Rp)"
FROM restaurant.employee_shift_performance
WHERE attendance_date >= (SELECT MAX(attendance_date) - INTERVAL 30 DAY FROM restaurant.employee_shift_performance)
GROUP BY employee_name
ORDER BY "Total Revenue (Rp)" DESC
```

<!-- MASTER ROSTER -->
<div class="diagnostics-header" style="margin-bottom: 24px;">
  <div class="diagnostics-eyebrow">📋 PERSONNEL DATABASE</div>
<h2 class="diagnostics-title">Master Roster</h2>
...
<div style="background: transparent; border-radius: 16px; border: 1px solid var(--color-border-primary, #e2e8f0); padding: 20px; box-shadow: none; margin-bottom: 64px;">
  <DataTable data={master_roster} search="true" rows=15 download="true">
    ...
    <Column id="Role" />
    <Column id="Primary Location" />
    <Column id="Total Shifts" align="center" />
    <Column id="Total Overtime (Hours)" align="center" />
    <Column id="Disciplinary Incidents" align="center" />
    <Column id="Base Salary (Rp)" align="right" fmt="#,##0" />
    <Column id="Total Revenue (Rp)" align="right" fmt="#,##0" />
  </DataTable>
</div>
...
<div class="diagnostics-header" style="margin-top: 24px; margin-bottom: 32px;">
  <div class="diagnostics-eyebrow">🗂️ DATA DIRECTORY & REPORT CARD</div>
<h2 class="diagnostics-title">Employee Profile & Performance Evaluation</h2>
  <p class="diagnostics-copy">Instant access to individual employee scorecards. Displays aggregated performance data, schedule adherence, and employee rankings for the <strong>Trailing 30 Days</strong>.</p>
</div>
...
SELECT 
    ...
    CASE attendance_status
        ...
        WHEN 'leave' THEN '⛱️ Leave'
    END as "Status",
    overtime_hours as "Overtime",
    orders_handled as "Orders",
...
<script>
  ...
  $: maxDateObj = emp_history && emp_history.length > 0 && emp_history[0].Date ? new Date(emp_history[0].Date) : new Date();
  ...
  $: activeEmpHistoryFull = emp_history ? emp_history.filter(h => h.employee_name === selectedEmpName).map(h => {
      ...
      let isAbsent = h.Status === '🪑 Absent' || h.Status === '⛱️ Leave';
      
      return {
          ...h,
          Date_Fmt: tglStr,
          Pesanan_Real: isAbsent ? null : h.Orders,
          Omzet_Real: isAbsent ? null : h.Revenue,
          Lembur_Real: (isAbsent || h.Overtime === 0) ? null : h.Overtime
      };
  }) : [];
  ...
  $: activeCalendar = Array.from({length: 30}, (_, i) => {
      ...
      else if (shift.Overtime > 0) { color = '#8b5cf6'; label = `Overtime ${shift.Overtime}h`; }
  ...
  <div class="history-section" style="background: transparent; border-radius: 20px; border: 1.5px solid var(--color-border-primary, rgba(99, 102, 241, 0.18)); padding: 24px; box-shadow: none; margin-top: 32px;">
    ...
    <div class="streak-container">
      ...
      <div class="streak-legend">
        <div class="legend-item"><div class="legend-color" style="background:#10b981;">{countHadir}</div> Present</div>
        <div class="legend-item"><div class="legend-color" style="background:#8b5cf6;">{countLembur}</div> Overtime</div>
        <div class="legend-item"><div class="legend-color" style="background:#f59e0b;">{countTerlambat}</div> Late</div>
        <div class="legend-item"><div class="legend-color" style="background:#ef4444;">{countAbsen}</div> Absent</div>
        <div class="legend-item"><div class="legend-color" style="background:#94a3b8;">{countLibur}</div> Off</div>
      </div>
    </div>
      ...
      <table style="width:100%; border-collapse:collapse; font-size:0.88rem; text-align:left;">
        ...
        <tbody>
          ...
          <tr style="border-bottom:1px solid var(--color-border-tertiary, #e2e8f0);">
            <td style="padding:10px 14px; font-weight:normal; color:var(--color-text-secondary, #475569);">{row.Date_Fmt}</td>
            ...
            <td style="padding:10px 14px; text-align:right; font-family:monospace; font-weight:700;">{row.Omzet_Real !== null ? Intl.NumberFormat('en-US').format(row.Omzet_Real) : '-'}</td>
