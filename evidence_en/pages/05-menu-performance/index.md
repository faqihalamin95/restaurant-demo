...
.branch-card-link {
  ...
  background: linear-gradient(160deg, rgba(59, 130, 246, 0.04), rgba(99, 102, 241, 0.015));
  box-shadow: 0 2px 8px rgba(0, 0, 0, 0.02), 0 1px 3px rgba(0, 0, 0, 0.01);
  transition: all 0.25s cubic-bezier(0.4, 0, 0.2, 1);
  cursor: pointer;
}
.branch-card-link:hover {
  ...
  background: linear-gradient(160deg, rgba(59, 130, 246, 0.08), rgba(99, 102, 241, 0.03));
  box-shadow: 0 10px 22px rgba(59, 130, 246, 0.1), 0 3px 6px rgba(0, 0, 0, 0.03);
}
</style>
...
{@const periodLensTitle = activePeriod === 'y' ? 'Daily Operational Check' : activePeriod === '30d' ? 'Initial Portfolio Decision' : 'Weekly Signal'}
...
<div class="hero" style="margin-top: 10px;">
  <div class="hero-eyebrow">🍔 Menu Performance ({activePeriod === '30d' ? 'Last 30 Days' : '7-Day Pattern'}) · {activePeriod === '30d' ? menu_dates[0].tgl_30_awal + ' - ' + menu_dates[0].tgl_akhir : menu_dates[0].tgl_7_awal + ' - ' + menu_dates[0].tgl_akhir}</div>
  <div class="hero-grid">
    <div class="hero-main-card {heroStatusClass}">
      <div class="hero-stat-number">{activeStableCount}/{activeMenuCount}</div>
      <div class="hero-stat-label">Stable Menu Items</div>
      <div class="hero-subtitle">
        {#if menuDeclineState === 'safe'}
          The majority of the <strong>{activeMenuCount} active menu items</strong> maintain a stable sales volume baseline.
        {:else if menuDeclineState === 'warn'}
          <strong>{activeDeclining} of {activeMenuCount} active menu items</strong> reflect a sales volume decline exceeding 20%.
        {:else}
          <strong>{activeDeclining} of {activeMenuCount} active menu items</strong> reflect a sales volume decline exceeding 20%.
        {/if}
        <div style="margin-top: 8px; font-size: 0.75em; opacity: 0.65;">
          *Excludes inactive items (zero order volume) from the active count. Volume shifts in zero-volume items are excluded from total decline metrics.
        </div>
      </div>
    </div>
    <div class="hero-side">
      <div class="hero-side-card">
        <div class="hero-side-label">💸 Revenue Leader</div>
        <div class="hero-side-value">{activeTopRevenue ?? 'No data yet'}</div>
        <div class="hero-side-note">The menu item generating the highest total gross revenue within the observed period.</div>
      </div>
      <div class="hero-side-card">
        <div class="hero-side-label">🍽️ Volume Leader</div>
        <div class="hero-side-value">{activeTopVolume ?? 'No data yet'}</div>
        <div class="hero-side-note">The menu item recording the highest total sales volume by order quantity.</div>
      </div>
    </div>
  </div>
</div>

<details class="guide-acc" style="margin-top: 12px; margin-bottom: 24px;">
  <summary>💡 Stable Menu Item Calculation Methodology</summary>
  <div class="guide-body">
    <p style="margin-top: 4px; margin-bottom: 16px; font-weight: 500; color: var(--color-text-secondary);">
      Measures trailing sales volume momentum by evaluating period-over-period item sales quantity against historical baselines (e.g., trailing 30 days vs. prior 30 days).
    </p>
    <div class="guide-grid" style="grid-template-columns: repeat(3, minmax(0, 1fr)); gap: 12px;">
      <div class="guide-card blue">
        <div class="guide-card-icon">🧮</div>
        <div class="guide-card-content">
          <div class="guide-card-label">Denominator</div>
          <h4 class="guide-card-title">Active Menu Items</h4>
          <p class="guide-card-desc">Evaluates menu items recording a minimum of 1 unit sold during the current period. Inactive items (zero order volume) are excluded from the baseline.</p>
        </div>
      </div>
      <div class="guide-card orange">
        <div class="guide-card-icon">📉</div>
        <div class="guide-card-content">
          <div class="guide-card-label">Variance Threshold</div>
          <h4 class="guide-card-title">20% Decline Baseline</h4>
          <p class="guide-card-desc">Flagged as a declining item when sales volume drops by more than 20% period-over-period. Low-volume passive items are excluded to prevent minor unit variances from skewing aggregate stability.</p>
        </div>
      </div>
      <div class="guide-card teal">
        <div class="guide-card-icon">📈</div>
        <div class="guide-card-content">
          <div class="guide-card-label">Classification</div>
          <h4 class="guide-card-title">Stable Menu Items</h4>
          <p class="guide-card-desc">Includes all menu items demonstrating volume growth, flat performance, or minor contractions (20% or less) period-over-period.</p>
        </div>
      </div>
    </div>
  </div>
...
SELECT 
  ...
  rr.rev as top_rev,
  CASE 
    WHEN a.passive_menus >= 3 THEN 'There are ' || a.passive_menus || ' passive menus. Evaluate daily material stock efficiency.'
    WHEN a.avg_item_price > 25000 THEN 'Customers prefer premium menus. Focus on upselling bundle packages.'
    ELSE 'Casual/economical snacking customer type. High volume in single menus.'
  END as insight
FROM agg a
...
WHERE a.branch_name != 'All Locations'
...
<div id="makro-fix">
  <div style="margin-bottom: 16px; display: flex; align-items: center; gap: 10px; margin-top: 48px; padding-top: 32px; border-top: 2px dashed rgba(0,0,0,0.06);">
    ...
    <div>
      <h2 style="margin: 0; font-size: 1.25rem; font-weight: 800; color: var(--color-text-primary); letter-spacing: -0.02em;">LOCAL CHARACTERISTICS PROFILE (30D)</h2>
      <div style="font-size: 0.85rem; color: var(--color-text-tertiary); font-weight: 500;">Analyzes location-specific menu preferences and unit-level sales velocity.</div>
    </div>
  </div>
  ...
  <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(320px, 1fr)); gap: 16px; margin-bottom: 40px;">
    {#each branch_local_profile as b}
      <a href="/05-menu-performance/02-deepdive?focus_branch={b.branch_name}" class="branch-card-link">
        ...
        <div class="kpi-grid-2" style="margin-bottom: 24px;">
          <div>
            <div style="color: var(--color-text-secondary); font-size: 0.75rem; margin-bottom: 4px;">👑 Volume Leader</div>
            <div style="font-weight: 700; color: var(--color-text-primary); font-size: 0.9rem; line-height: 1.3;">{b.top_qty_menu}</div>
            <div style="font-size: 0.75rem; font-weight: 600; color: #16a34a; margin-top: 4px;">{Number(b.top_qty).toLocaleString('en-US')} units sold</div>
          </div>
          <div>
            <div style="color: var(--color-text-secondary); font-size: 0.75rem; margin-bottom: 4px;">💸 Revenue Leader</div>
            <div style="font-weight: 700; color: var(--color-text-primary); font-size: 0.9rem; line-height: 1.3;">{b.top_rev_menu}</div>
            <div style="font-size: 0.75rem; font-weight: 600; color: var(--color-text-tertiary); margin-top: 4px;">Rp {Number(b.top_rev / 1000000).toLocaleString('en-US', {minimumFractionDigits:1, maximumFractionDigits:1})}m</div>
          </div>
        </div>

        <div style="display: flex; gap: 8px; margin-bottom: 16px; background: var(--color-background-secondary); padding: 12px; border-radius: 8px; border: 1px solid rgba(0,0,0,0.03);">
          <div style="flex: 1; display: flex; flex-direction: column; justify-content: space-between;">
            <div style="color: var(--color-text-secondary); font-size: 0.7rem; text-transform: uppercase; font-weight: 700; margin-bottom: 4px;">Avg Item Price</div>
            <div style="font-size: 0.95rem; font-weight: 800; color: #2563eb;">Rp {Math.round(b.avg_item_price).toLocaleString('en-US')}</div>
          </div>
          <div style="width: 1px; background: rgba(128,128,128,0.2);"></div>
          <div style="flex: 1; display: flex; flex-direction: column; justify-content: space-between;">
            <div style="color: var(--color-text-secondary); font-size: 0.7rem; text-transform: uppercase; font-weight: 700; margin-bottom: 4px;">Active Menu</div>
            <div style="font-size: 0.95rem; font-weight: 800; color: #16a34a;">
              {b.active_menus} <span style="font-size: 0.75rem; color: var(--color-text-tertiary); font-weight: 600;">Items</span>
            </div>
          </div>
          <div style="width: 1px; background: rgba(128,128,128,0.2);"></div>
          <div style="flex: 1; display: flex; flex-direction: column; justify-content: space-between;">
            <div style="color: var(--color-text-secondary); font-size: 0.7rem; text-transform: uppercase; font-weight: 700; margin-bottom: 4px;">Inactive Menu</div>
            <div style="font-size: 0.95rem; font-weight: 800; color: {b.passive_menus > 0 ? '#dc2626' : 'var(--color-text-primary)'};">
              {b.passive_menus} <span style="font-size: 0.75rem; color: var(--color-text-tertiary); font-weight: 600;">Items</span>
            </div>
          </div>
        </div>
  ...
  <div style="margin-bottom: 16px; display: flex; align-items: center; gap: 10px; margin-top: 48px; padding-top: 32px; border-top: 2px dashed rgba(0,0,0,0.06);">
    ...
    <div>
      ...
      <div style="font-size: 0.85rem; color: var(--color-text-tertiary); font-weight: 500;">Evaluates menu portfolio revenue concentration and item inactivity metrics.</div>
    </div>
  </div>
...
<div id="makro-fix">
  <div class="kpi-grid-2">
    <div class="kpi-card revenue">
      <div class="kpi-label">⚖️ Revenue Concentration (CR5)</div>
      <div class="kpi-value">{activeTop5Share}%</div>
      <div class="kpi-meta">
        <span class="trend-indicator neutral">Top 5 Item Revenue Share</span>
      </div>
      <div class="kpi-prev">Top 5 menu items generate {activeTop5Share}% of total gross revenue.</div>
    </div>
    
    <div class="kpi-card expense">
      <div class="kpi-label">🧊 Inactive Menu Items</div>
      <div class="kpi-value">{passiveCount}</div>
      <div class="kpi-meta">
        <span class="trend-indicator {passiveCount === 0 ? 'up' : 'down'}" style="color: {passiveCount === 0 ? '#15803d' : '#b91c1c'};">Zero-Volume Items</span>
      </div>
      <div class="kpi-prev">Items recording zero sales volume during the observed period.</div>
    </div>
  </div>
</div>
  ...
  <div class="clean-cta-banner">
    <div class="clean-cta-content">
      ...
      <div class="clean-cta-text">
        <h3 class="clean-cta-title">Menu Performance Analytics</h3>
        <p class="clean-cta-desc">Evaluates item velocity trends, operational performance anomalies, and margin contribution variance across active menu categories.</p>
      </div>
    </div>
    <a href="/05-menu-performance/03-analysis" class="clean-cta-button">
      Open Menu Evaluation ➔
    </a>
  </div>
</div>
