...
# 1. Extract Section 2 (Hari Sibuk)
# Section 2 starts around <div style="margin-top: 32px; margin-bottom: 32px;"> containing Analisis Volume Hari Sibuk
sec2_pattern = r'(\s*<div style="margin-top: 32px; margin-bottom: 32px;">\s*<DiagnosticsHeader\s+marginTop="24px"\s+eyebrow="📅 MAKRO"\s+title="Analisis Volume Hari Sibuk".*?<\/div>\s*<\/div>\s*<\/div>\s*)'

sec2_match = re.search(sec2_pattern, content, re.DOTALL)
if not sec2_match:
    print("Could not find Section 2")
    exit(1)

sec2_html = sec2_match.group(1)

# Remove Section 2 from its current location inside Tab 1
content_without_sec2 = content.replace(sec2_html, '\n\n')

# 2. Rename Tab 1 label to "⏰ Jam Sibuk"
content_without_sec2 = content_without_sec2.replace('<Tab label="⏰ Jam &amp; Hari Sibuk">', '<Tab label="⏰ Jam Sibuk">')
content_without_sec2 = content_without_sec2.replace('<Tab label="⏰ Jam & Hari Sibuk">', '<Tab label="⏰ Jam Sibuk">')

# 3. Construct Tab 2 "📅 Hari Sibuk" HTML wrapper
tab2_html = f"""    </Tab>
...
new_content = content_without_sec2.replace('<Tab label="🔁 Tren Musiman">', tab2_html + '    <Tab label="🔁 Tren Musiman">')

with open('/home/faqih/projects/restaurant-demo/evidence/pages/04-peak-hours/deepdive.md', 'w') as f:
    f.write(new_content)

print("Split into 3 tabs successfully!")
