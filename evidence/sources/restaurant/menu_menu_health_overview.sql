...
prev_y   AS (SELECT menu_name, SUM(total_qty_sold) AS qty FROM main_marts.mart_menu_performance CROSS JOIN max_d WHERE order_date = d - INTERVAL '7 days' GROUP BY menu_name),
prev_7d  AS (SELECT menu_name, SUM(total_qty_sold) AS qty FROM main_marts.mart_menu_performance CROSS JOIN max_d WHERE order_date >= d - INTERVAL '13 days' AND order_date < d - INTERVAL '6 days' GROUP BY menu_name),
prev_30d AS (SELECT menu_name, SUM(total_qty_sold) AS qty FROM main_marts.mart_menu_performance CROSS JOIN max_d WHERE order_date >= d - INTERVAL '59 days' AND order_date < d - INTERVAL '29 days' GROUP BY menu_name),
