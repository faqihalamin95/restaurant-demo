...
period_stats AS (
    SELECT
        CASE
            WHEN order_date >= (SELECT d FROM anchor_date) - INTERVAL '6 days' THEN 'current'
            WHEN order_date >= (SELECT d FROM anchor_date) - INTERVAL '13 days'
             AND order_date < (SELECT d FROM anchor_date) - INTERVAL '6 days' THEN 'previous'
        END AS period,
        branch_name,
...
SELECT
    c.*,
    p.total_orders_prev,
    ROUND((c.total_orders_all - p.total_orders_prev) / NULLIF(p.total_orders_prev, 0) * 100, 1) AS pct_change_orders_7d
