...
WITH low AS (
    SELECT *
    FROM ${inv_reorder_items}
    WHERE days_remaining < 3
),
over AS (
    SELECT *
    FROM ${inv_overstock_items}
...
```sql inv_action_board
WITH low_actions AS (
    SELECT
        1 AS priority_group,
        CASE WHEN days_remaining < 1.5 THEN 'critical' ELSE 'high' END AS severity_class,
        reorder_status AS action_type,
        'Reorder ' || item_name || ' di ' || branch_name AS action_title,
        'Stok tersisa ' || stock_on_hand || ' ' || unit || ', estimasi habis dalam ' || days_remaining || ' hari. Suggested reorder untuk coverage 7 hari: ' || suggested_reorder_qty || ' ' || unit || '.' AS action_detail,
        'Hari ini' AS action_window,
        stock_value AS impact_value
    FROM ${inv_reorder_items}
...
transfer_actions AS (
    SELECT
        2 AS priority_group,
        'medium' AS severity_class,
        'Transfer Antar Cabang' AS action_type,
        'Transfer ' || item_name || ': ' || branch_source || ' ke ' || branch_need AS action_title,
        branch_need || ' tinggal ' || need_days || ' hari, sementara ' || branch_source || ' punya coverage ' || source_days || ' hari. Validasi fisik sebelum transfer.' AS action_detail,
        'Hari ini' AS action_window,
        source_stock_value AS impact_value
    FROM ${inv_transfer_candidates}
    LIMIT 4
),
overstock_actions AS (
    SELECT
        3 AS priority_group,
        CASE WHEN estimated_idle_value >= 1000000 THEN 'high' ELSE 'medium' END AS severity_class,
        'Tahan Pembelian' AS action_type,
        'Tahan pembelian ' || item_name || ' di ' || branch_name AS action_title,
        'Coverage ' || days_remaining || ' hari, sekitar ' || days_to_normal || ' hari di atas batas normal. ' || recommended_action || '. Estimasi modal idle Rp ' || estimated_idle_value || '.' AS action_detail,
        'Minggu ini' AS action_window,
        estimated_idle_value AS impact_value
    FROM ${inv_overstock_items}
    ORDER BY estimated_idle_value DESC
    LIMIT 6
),
price_actions AS (
    SELECT
        4 AS priority_group,
        CASE WHEN severity = 'Kritis' THEN 'high' ELSE 'medium' END AS severity_class,
        'Renegosiasi Supplier' AS action_type,
        'Renegosiasi harga ' || item_name AS action_title,
        'Harga rata-rata Rp ' || avg_unit_cost || ' vs dasar Rp ' || base_unit_cost || ' (+' || price_variance_pct || '%). Estimasi tekanan biaya 30H: Rp ' || estimated_price_impact || '.' AS action_detail,
        '2 Minggu' AS action_window,
        estimated_price_impact AS impact_value
    FROM ${inv_supplier_alerts}
    ORDER BY estimated_price_impact DESC
    LIMIT 6
)
SELECT * FROM low_actions
UNION ALL
SELECT * FROM transfer_actions
UNION ALL
SELECT * FROM overstock_actions
UNION ALL
SELECT * FROM price_actions
ORDER BY priority_group, impact_value DESC
```
...
<div class="evidence-tabs-container">
  ...
  <a href="/03-inventori-stok/action" class="tab-button active">🎯 Pusat Aksi</a>
</div>


<div class="subpage-hero">
  <div class="subpage-hero-eyebrow">🎯 PUSAT AKSI INVENTORI</div>
  <h3 class="subpage-hero-title">Rekomendasi Tindakan Operasional &amp; Efisiensi</h3>
  <p class="subpage-hero-copy">Urutan prioritas tindakan dirancang dari risiko operasional (habis stok) hingga potensi penghematan modal (overstock &amp; supplier).</p>
</div>

<details class="context-acc" open>
  <summary>💡 Metodologi Prioritas Aksi</summary>
  <div class="acc-body">
    Aksi diurutkan berdasarkan tingkat urgensi:
    <ul>
      <li><strong>Prioritas 1 (Reorder)</strong>: Mencegah stockout menu di outlet. Harus segera dipesan atau ditransfer.</li>
      <li><strong>Prioritas 2 (Transfer)</strong>: Memanfaatkan stok berlebih dari cabang lain untuk menambal cabang yang kritis sebelum membeli baru.</li>
      <li><strong>Prioritas 3 (Tahan PO)</strong>: Menunda pengadaan untuk item yang overstock guna melepaskan modal kerja yang macet.</li>
      <li><strong>Prioritas 4 (Negosiasi)</strong>: Melakukan negosiasi harga dengan supplier untuk item dengan kenaikan harga signifikan.</li>
    </ul>
  </div>
</details>

{#if inv_action_board.length > 0}
<div class="inv-page">
  <div class="action-stack" style="margin-top: 16px;">
    {#each inv_action_board as a}
      <div class="action-card {a.severity_class}">
        <div class="action-header">
          <div class="action-workline">
            <span class="action-rank">{a.priority_group}</span>
            <span class="action-severity {a.severity_class}">{a.action_type}</span>
            <span class="action-badge">{a.action_window}</span>
          </div>
          <div class="action-impact">
            {#if a.impact_value > 0}
              Rp {a.impact_value.toLocaleString('id-ID')}
              <span>estimasi dampak</span>
            {/if}
          </div>
        </div>
        <div class="action-title">{a.action_title}</div>
        <div class="action-copy">{a.action_detail}</div>
      </div>
    {/each}
  </div>
</div>
...
<div class="inv-page">
  <div class="action-empty" style="margin-top: 10px;">
    <div class="title">✅ Semua sistem berjalan normal. Tidak ada tindakan prioritas merah hari ini.</div>
  </div>
</div>
{/if}
