...
<div class="evidence-tabs-container">
  ...
  <a href="/03-inventori-stok/reorder" class="tab-button active">🛒 Reorder</a>
...
{#if inv_reorder_items.length > 0}
  {@const kritisCount = inv_reorder_items.filter(row => row.days_remaining < 1.5).length}
  {@const reorderCount = inv_reorder_items.filter(row => row.days_remaining >= 1.5 && row.days_remaining < 3.0).length}
  {@const pantauCount = inv_reorder_items.filter(row => row.days_remaining >= 3.0).length}
<div class="inv-page">
  ...
  <details class="context-acc" open>
    ...
    <div class="acc-body">
      Subpage ini menampilkan item-item yang stoknya hampir habis (estimasi coverage di bawah 5 hari). Prioritas diurutkan dari coverage paling pendek agar tim pengadaan tahu barang apa yang harus dibeli atau dipindahkan lebih dulu.
      <ul>
        <li><strong>Kritis Hari Ini (days_remaining &lt; 1.5 hari)</strong>: Risiko tinggi menu tidak dapat dijual dalam 24 jam ke depan. Tindakan darurat diperlukan.</li>
        <li><strong>Reorder Sekarang (days_remaining &lt; 3.0 hari)</strong>: Segera buat purchase order (PO) atau transfer stok dari cabang lain.</li>
        <li><strong>Pantau Minggu Ini (days_remaining &lt;= 5.0 hari)</strong>: Masukkan dalam jadwal PO berikutnya untuk mengantisipasi kenaikan volume.</li>
      </ul>
    </div>
  </details>
  ...
  {#if inv_transfer_candidates.length > 0}
  <div class="section-card" style="margin-top: 16px;">
    <div class="section-head">
