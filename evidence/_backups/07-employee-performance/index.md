...
<div class="hero" style="margin-bottom: 32px; margin-top: 10px;">
  <div class="hero-eyebrow">👥 Performa Pegawai · {tglAwal} - {tglAkhir}</div>
  <div class="hero-grid">
    <div class="hero-main-card {heroStatusClass}">
      <div class="hero-stat-number">{idFormat(attendanceRate, 1)}%</div>
      <div class="hero-stat-label">TINGKAT KEHADIRAN</div>
      <div class="hero-subtitle">
        {#if heroStatusClass === 'status-sehat'}
          Mayoritas jadwal shift terpenuhi secara optimal bulan ini.
        {:else if heroStatusClass === 'status-waspada'}
          Kehadiran cukup baik, namun waspadai beberapa shift kosong.
        {:else}
          Banyak shift kosong yang berisiko mengganggu operasional.
        {/if}
      </div>
    </div>
    <div class="hero-side">
      <div class="hero-side-card">
        <div class="hero-side-label">⏱️ Ekstra Jam (Lembur)</div>
        <div class="hero-side-value">{idFormat(overtimeHours)} Jam</div>
        <div class="hero-side-note">Total jam kerja tambahan yang harus dipikul staf untuk menutupi shift.</div>
      </div>
      <div class="hero-side-card">
        <div class="hero-side-label">⚠️ Telat & Absen</div>
        <div class="hero-side-value">{idFormat(lateCount)} Telat | {idFormat(absentCount)} Absen</div>
        <div class="hero-side-note">Catatan kedisiplinan jadwal yang berisiko mengganggu kelancaran layanan.</div>
      </div>
    </div>
  </div>
</div>
...
<div style="margin-top: 32px; margin-bottom: 20px; display: flex; align-items: center; gap: 10px;">
  ...
  <div>
    ...
    <div style="font-size: 0.85rem; color: var(--color-text-tertiary); font-weight: 500;">Fokus: Eksekusi Operasional SDM yang Harus Dikerjakan Hari Ini</div>
  </div>
</div>
...
<div class="menu-health">
  ...
  <div class="menu-health-list">
    <a href="/07-employee-performance/02-aksi-taktis#disiplin" style="text-decoration: none; color: inherit;">
      <div class="menu-health-row {lateState}">
        <div class="menu-health-icon">{lateState === 'safe' ? '✅' : lateState === 'warn' ? '⚠️' : '🚨'}</div>
        <div style="width: 100%;">
          <span class="menu-health-title">Pelanggaran Disiplin</span> 
          <span class="menu-health-copy">- <span class="menu-health-value">{problemEmployees} staf terdeteksi indisipliner parah.</span> Segera jadwalkan 1-on-1 coaching atau terbitkan Surat Peringatan (SP).</span>
        </div>
      </div>
    </a>
    <a href="/07-employee-performance/02-aksi-taktis#burnout" style="text-decoration: none; color: inherit;">
      <div class="menu-health-row {overtimeState}">
        <div class="menu-health-icon">{overtimeState === 'safe' ? '✅' : overtimeState === 'warn' ? '⚠️' : '🚨'}</div>
        <div style="width: 100%;">
          <span class="menu-health-title">Risiko Burnout (Kelelahan)</span> 
          <span class="menu-health-copy">- <span class="menu-health-value">{burnoutEmployees} staf bekerja dengan beban lembur tinggi (>10 jam).</span> Pertimbangkan penyesuaian roster shift untuk menjaga keseimbangan beban kerja.</span>
        </div>
      </div>
    </a>
  </div>
...
<details class="guide-acc" style="margin-top: 16px; margin-bottom: 24px;">
  <summary>💡 Kenapa tingkat disiplin dan lembur jadi angka utama?</summary>
  <div class="guide-body" style="padding: 16px;">
    <p style="margin-top: 4px; margin-bottom: 16px; font-weight: 500; color: var(--color-text-secondary);">
      Dua metrik ini bertindak sebagai indikator awal masalah operasional yang berujung pada lambatnya pelayanan dan membengkaknya biaya.
    </p>
    <div class="guide-grid" style="grid-template-columns: repeat(auto-fit, minmax(280px, 1fr)); gap: 16px;">
      <div class="guide-card blue">
        ...
        <div class="guide-card-content">
          <div class="guide-card-label">Keterlambatan & Absensi</div>
          <h4 class="guide-card-title">Efek Domino Pelayanan</h4>
          <p class="guide-card-desc">Satu staf kasir telat 15 menit saat jam sibuk berisiko antrean panjang dan menurunkan tingkat kepuasan pelanggan secara instan.</p>
        </div>
      </div>
      <div class="guide-card purple">
        <div class="guide-card-icon">🥵</div>
        <div class="guide-card-content">
          <div class="guide-card-label">Overtime Tinggi</div>
          <h4 class="guide-card-title">Bahaya Burnout Staf</h4>
          <p class="guide-card-desc">Shift lembur menutupi absen, tapi jangka panjang merusak fokus staf dan berdampak pada human-error tinggi (pesanan salah, komplain naik).</p>
        </div>
      </div>
    </div>
  </div>
...
<div style="margin-bottom: 16px; display: flex; align-items: center; gap: 10px; margin-top: 48px; padding-top: 32px; border-top: 2px dashed rgba(0,0,0,0.06);">
  ...
  <div>
    ...
    <div style="font-size: 0.85rem; color: var(--color-text-tertiary); font-weight: 500;">Fokus: Evaluasi Kebijakan SDM Jangka Panjang</div>
  </div>
</div>
...
<div class="kpi-grid-2" style="margin-bottom: 12px;">
  <div class="kpi-card margin">
    <div class="kpi-label">🚨 Shift Paling Rawan</div>
    <div class="kpi-value">{pressureShift}</div>
    <div class="kpi-prev" style="margin-top: 12px;">Jadwal dengan tingkat kedisiplinan terburuk bulan ini.</div>
  </div>
  <div class="kpi-card margin">
    <div class="kpi-label">🏢 Cabang Paling Rawan</div>
    <div class="kpi-value">{pressureBranch}</div>
    <div class="kpi-prev" style="margin-top: 12px;">Lokasi yang paling krisis kekurangan staf/disiplin.</div>
  </div>
</div>
<div class="kpi-grid" style="margin-bottom: 24px;">
  <div class="kpi-card revenue">
    <div class="kpi-label">💰 Revenue per Jam</div>
    <div class="kpi-value">Rp {idFormat(revPerHour)}</div>
    <div class="kpi-prev" style="margin-top: 12px;">Rata-rata produktivitas rupiah per jam kerja staf.</div>
  </div>
  <div class="kpi-card expense">
    <div class="kpi-label">🕒 Beban Lembur (%)</div>
    <div class="kpi-value">{idFormat(overtimePct, 1)}%</div>
    <div class="kpi-prev" style="margin-top: 12px;">Persentase ketergantungan operasional pada jam ekstra.</div>
  </div>
  <div class="kpi-card expense">
    <div class="kpi-label">📉 Tingkat Indisipliner (%)</div>
    <div class="kpi-value">{idFormat(latePct, 1)}%</div>
    <div class="kpi-prev" style="margin-top: 12px;">Rasio insiden keterlambatan operasional bulan ini.</div>
  </div>
</div>
...
<div class="clean-cta-banner">
  <div class="clean-cta-content">
    ...
    <div class="clean-cta-text">
      <h3 class="clean-cta-title">Eksplorasi Strategi Promosi & Rotasi</h3>
      <p class="clean-cta-desc">Bedah lebih dalam efektivitas jadwal, pergerakan tren disiplin, dan temukan celah rotasi yang lebih efisien.</p>
    </div>
  </div>
  <a href="/07-employee-performance/03-evaluasi-strategis" class="clean-cta-button">
